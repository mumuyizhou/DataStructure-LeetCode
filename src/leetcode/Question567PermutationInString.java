 ...
 * @author 木木漪
 */
public class Question567PermutationInString {
	public boolean checkInclusion(String s1, String s2) {
		int lenS1 = s1.length();
		int lenS2 = s2.length();
		int[] countS1 = new int[123];//acsii码范围97-122;
		for (int i = 0; i < lenS1; i++) {
			countS1[(int) s1.charAt(i)]++;
		}
		int[] countSubS2 = new int[123];
		int numOfContinueInS1 = 0;
		for (int i = 0; i < lenS2; i++) {
			if (countS1[(int) s2.charAt(i)] != 0) {
				countSubS2[(int) s2.charAt(i)]++;
				numOfContinueInS1++;
				if (numOfContinueInS1 == lenS1) {
					if (charAllEqual(countS1, countSubS2)) {
						return true;
					} else {
						numOfContinueInS1--;
						countSubS2[(int) s2.charAt(i - lenS1 + 1)]--;
					}

				}
			} else {
				numOfContinueInS1 = 0;
				for (int j = 97; j < countSubS2.length; j++) {
					countSubS2[j] = 0;
				}
			}
		}
		return numOfContinueInS1 == lenS1 && charAllEqual(countS1, countSubS2);
	}

	public boolean charAllEqual(int[] s1, int[] s2) {
		for (int i = 97; i < s1.length; i++) {
			if (s1[i] != s2[i]) {
				return false;
			}
		}
	...
	public static void main(String[] args) {
		Question567PermutationInString string = new Question567PermutationInString();
		System.out.println(string.checkInclusion("ky", "ainwkckifykxlribaypk"));
	}
}
